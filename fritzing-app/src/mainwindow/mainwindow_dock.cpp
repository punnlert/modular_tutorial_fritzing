...
static const int BlinkBoardWidgetMinHeight = 100;
static const int BlinkBoardWidgetDefaultHeight = 150;
...
void MainWindow::createDockWindows() {
  ...
  makeDock(BinManager::Title, m_binManager, PartsBinMinHeight,
           PartsBinHeightDefault /*, Qt::LeftDockWidgetArea*/);

  makeDock(tr("BlinkBoard Connection"), m_blinkboardWidget.data(),
           BlinkBoardWidgetMinHeight, BlinkBoardWidgetDefaultHeight)
      ->hide();

  makeDock(tr("Inspector"), m_infoView, InfoViewMinHeight,
           InfoViewHeightDefault);

  makeDock(tr("Undo History"), m_undoView, UndoHistoryMinHeight,
           UndoHistoryDefaultHeight)
      ->hide();
...
FDockWidget *MainWindow::makeDock(const QString &title, QWidget *widget,
                                  int dockMinHeight, int dockDefaultHeight,
                                  Qt::DockWidgetArea area,
                                  DockFactory dockFactory) {
  FDockWidget *dock =
      ((dockFactory) ? dockFactory(title, this) : new FDockWidget(title, this));
...
FDockWidget *MainWindow::dockIt(FDockWidget *dock, int dockMinHeight,
  ...
  connect(dock, SIGNAL(dockChangeActivationSignal(bool, QWidget *)), this,
          SLOT(dockChangeActivation(bool, QWidget *)));
  connect(dock, SIGNAL(destroyed(QObject *)), qApp,
          SLOT(topLevelWidgetDestroyed(QObject *)));
  connect(dock, SIGNAL(dockChangeActivationSignal(bool, QWidget *)), qApp,
          SLOT(changeActivation(bool, QWidget *)), Qt::DirectConnection);
...
void MainWindow::keepMargins() {
  ...
  if (m_bottomDock != newBottomWidget) {
    ...
    if (m_bottomDock)
      m_oldBottomDockStyle = m_bottomDock->styleSheet();
...
void MainWindow::dockMarginAux(FDockWidget *dock, const QString &name,
                               const QString &style) {
  if (dock) {
...
void MainWindow::dontKeepMargins() { m_dontKeepMargins = true; }
...
void MainWindow::initDock() {
  ...
  connect(m_infoView, SIGNAL(clickObsoleteSignal()), this,
          SLOT(selectAllObsolete()));
  // DebugDialog::debug("after html view");

  m_binManager =
      new BinManager(m_referenceModel, m_infoView, m_undoStack, this);
  m_binManager->initStandardBins();

  m_blinkboardWidget = new BlinkBoardWidget(this);
...
void MainWindow::restoreDocks() {
  for (int i = 0; i < children().count(); i++) {
    ...
    // DebugDialog::debug(QString("restoring dock
    // %1").arg(dock->windowTitle()));
    dock->restoreState();
  }
}
